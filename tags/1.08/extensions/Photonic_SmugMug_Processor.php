			...
			if (is_array($images) && count($images) > 0) {
				...
				if (isset($attr['display']) && $attr['display'] == 'popup') {
					...
					if ($photonic_smug_photo_pop_title_display == 'tooltip') {
						$ret .= "<script type='text/javascript'>\$j('.photonic-smug-panel a').each(function() { \$j(this).data('title', \$j(this).attr('title')); }); \$j('.photonic-smug-panel a').each(function() { var iTitle = \$j(this).find('img').attr('alt'); \$j(this).tooltip({ bodyHandler: function() { return iTitle; }, showURL: false });})</script>";
					}
